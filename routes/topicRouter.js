...
topicRouter.post("/", (req, res) => {
    ...
    db.get(
        "SELECT id FROM Topics WHERE name = ? AND date = ? AND ((timeStart BETWEEN ? AND ?) OR (timeEnd BETWEEN ? AND ?))",
        [className, teacherId, date, timeStart, timeEnd, timeStart, timeEnd],
        (err, row) => {
            if (err) {
                console.error(err);
            ...
            if (row) {
                return res.status(203).json({ message: "Lớp học đã tồn tại trong khoảng thời gian này." });
            }
